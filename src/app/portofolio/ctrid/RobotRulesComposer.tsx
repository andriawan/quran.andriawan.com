      ...
      ?.map((condition) => {
        if (condition.conditions) {
          const logic2 = condition.conditions
            .map((condition2) => {
              return `metric ${condition2?.metricText ?? "?"} for ${condition2?.periodText ?? "?"} is ${condition2?.operation ?? "?"} ${condition2?.value ? Intl.NumberFormat().format(Number(condition2.value)) : "?"}`;
            })
            .join(` ${condition.operation ?? "?"} `);
          return `(${logic2})`;
        }
        return `metric ${condition?.metricText ?? "?"} for ${condition?.periodText ?? "?"} is ${condition?.operation ?? "?"} ${condition?.value ? Intl.NumberFormat().format(Number(condition.value)) : "?"}`;
      })
      .join(` ${rule.condition.operation ?? "?"} `);
    return `this rule will ${actionState ?? "?"} the campaign when ${logic} `;
  };
  return (
    <div
      className={clsx(
